...
- **SCONE** — Kim et al. 2026 (Cambridge Nuclear) reference implementation of AMLG-Patch and its predecessors, four acceleration methods (`none`/`octree`/`patchSingle`/`patchMulti`), CPU + OpenMP (1 and 8 threads). Runs on the pad-fix sweep with `BOX_PAD=5`. SCONE has no query-list driver so `correct%` is not applicable (it ray-traces with internal validation).
