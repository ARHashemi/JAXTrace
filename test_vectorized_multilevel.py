...
def main():
    ...
    print("\n" + "=" * 80)
    print("VECTORIZED MULTI-LEVEL SEARCH TEST")
    print("=" * 80)
    ...
    # 50K particles can cause OOM on 4GB GPU when combined with mesh data
    ...
    for n_particles in particle_counts:
        ...
        seq_vram_delta = vram_end - vram_start
        ...
        print(f"  VRAM Δ:     {seq_vram_delta:+.1f} MB")
        print(f"\n  Hit Rates:")
        ...
        mem_info = get_gpu_memory_info()
        vram_end = mem_info.used_mb
        vec_vram_delta = vram_end - vram_start
        ...
        print(f"  VRAM Δ:     {vec_vram_delta:+.1f} MB")
        print(f"\n  Hit Rates:")
        ...
        print(f"\n  Correctness Check:")
        matches = np.sum(vec_elem_ids == seq_elem_ids)
        ...
        results.append({
            ...
            'speedup': speedup,
            'seq_vram': seq_vram_delta,
            'vec_vram': vec_vram_delta,
            'match_rate': match_rate,
        })

        # Clear GPU memory between tests to prevent OOM on large particle counts
        print(f"\n🧹 Clearing GPU memory...")
        jax.clear_caches()
        import gc
        gc.collect()
