#!/usr/bin/env python3
"""
JAXTrace GPU-Optimized Workflow with FEM Interpolation

Uses finite element method for accurate and fast interpolation:
- Tetrahedral element lookup using spatial hash grid
- Barycentric coordinate interpolation
- Fully JIT-compiled on GPU
- More accurate than nearest-neighbor
- Potentially faster due to better cache locality
"""
...
print("="*80)
print(f"JAXTrace v{jt.__version__} - FEM Interpolation Workflow")
print("="*80)


def main():
    """Workflow with FEM interpolation."""
    ...
    field = load_vtk_with_fem()

    # 3. Track
    print("\n3. GPU-ACCELERATED TRACKING (FEM)")
    print("="*80)

    trajectory = fast_fem_tracking(field)
    ...
    print("\n" + "="*80)
    print("✅ FEM WORKFLOW COMPLETE!")
    print("="*80)


def load_vtk_with_fem():
    """Load VTK data with mesh connectivity for FEM interpolation."""
    ...
    # Create FEM field
    print(f"🔨 Creating FEM interpolation field...")
    ...
    field = FEMTimeSeriesField(
        ...
        fem_grid_resolution=256  # Very high resolution for flat anisotropic domain
    )
...
def fast_fem_tracking(field):
    """Fast tracking with FEM interpolation."""
    ...
    print(f"   Interpolation: FEM (tetrahedral)")
    ...
    # Track
    print(f"\n🏃 Running FEM tracking...")
    print(f"   Expect 10-30s for first JIT compilation")
...
def export_results(trajectory):
    ...
    try:
        ...
        export_trajectory_to_vtk(
            ...
            filename=str(output_dir / "trajectory_fem.vtp"),
            include_velocities=False,
            time_series=False
        )
        print(f"   ✅ Saved: output/trajectory_fem.vtp")
