...
# =============================================================================
# USER CONFIGURATION — Paper Sec.7 L0/L1/L2 hit-rate diagnostic (LUMI).
#
# This is a copy of scripts/run_lumi.sh with the paper-diagnostic knob
# HIT_STATS_LOG=1 pre-enabled, so a submission of this script produces
# a hit_stats.csv alongside the usual tracking outputs.  Fill in
# INPUT / MESH_SUBDIR / FEMUSS_SUBDIR / MESH_PATTERN / FEMUSS_PATTERN
# for whichever ROM/FOM case you want to characterise, then submit
# with `sbatch --account=project_XXXXXXXXX paper_benchmarks/run_lumi.sh`.
#
...
# every LOG_INTERVAL steps recording the L0/L1/L2/miss distribution
# aggregated over N_active × 5 RK4 sub-step searches for that step.
# Adds one extra RK4 step per log tick (negligible against the RK4
# budget).  This copy of run_lumi.sh has it enabled by default — set
# to 0 to run the same config without the diagnostic.
HIT_STATS_LOG=1
