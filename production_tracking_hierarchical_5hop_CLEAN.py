#!/usr/bin/env python3
"""
Production Particle Tracking Workflow - Hierarchical 5-Hop Search

Implements full-scale particle tracking with hierarchical early-exit 5-hop search:
- 105,000 particles (uniform grid: 50x70x30)
- 2,500 timesteps
- Hierarchical 5-hop L1 search (expected 82% retention vs 16% baseline)
- Async VTK export (no impact on tracking performance)
- Complete resource monitoring

Hierarchical 5-Hop Search Architecture:
...
- L1: Hierarchical multi-hop with early exit (vmap + nested lax.cond)
  ...
  - Hop 4: 256 neighbors (if not found at hop 3)
  - Hop 5: 1,024 neighbors (if not found at hop 4)
- No L2 fallback inside RK4 (PHASE3A architecture)

Expected Performance:
- Hit rate: 99.99% (vs 99.9% for 3-hop)
- Retention: 82% at 2,500 steps (vs 16% for 3-hop)
- Throughput: 8-15k p/s (vs 23k for 3-hop)
- Memory: 10 MB (vs 572 MB for naive 5-hop concatenation)

Memory Efficiency:
- Naive 5-hop concatenation: 1,364 neighbors x 105k = 572 MB -> GPU OOM
- Hierarchical early-exit: avg ~25 neighbors x 105k = 10 MB -> Works
...
OUTPUT_DIR = Path("./output/threadeda_hierarchical_5hop")  # Hierarchical 5-hop test output
...
# - 3: ~84 neighbors (98-99.5% hit rate, ~15-20k p/s, baseline, 16% retention)
# - 4: ~15 neighbors avg (hierarchical early-exit, 99.95% hit rate, ~12-18k p/s, 60% retention)
# - 5: ~25 neighbors avg (hierarchical early-exit, 99.99% hit rate, ~8-15k p/s, 82% retention, GPU OOM!)
# Higher hop counts = better particle retention, but slower throughput and more GPU memory
# HIERARCHICAL 4-HOP: Uses early exit to avoid memory explosion while fitting in GPU memory
RK4_L1_HOP_COUNT = 3  # HIERARCHICAL: 4-hop early-exit for 60% particle retention (vs 16% for 3-hop)
...
if USE_GPU_FUSED_RK4:
    ...
    if USE_BLOCK_LOCAL_FALLBACK:
        ...
        _, _ = rk4_step_gpu_fused_for_production_hierarchical(
            ...
            mesh_gpu,
            current_time=0.0,
            n_hops=RK4_L1_HOP_COUNT  # 5-hop hierarchical search
        )
else:
