...
(${\sim}21\%$). Active-subspace and manifold diagnostics show the response
is $\vadv$-dominated and lives on a smooth but curved low-dimensional
manifold, so the practical next step is more samples in the sparsely
covered high-$|\opin|$/low-$\vadv$ corner.
\end{abstract}
...
normalization, input features, nor regressor choice moves the needle; and
the active subspace is nearly 1D in $\vadv$. The limiting factor is
unambiguously \textbf{sample count and placement} --- 20 points in a 2D
space that is effectively 1D, sparsest exactly where the error peaks.
...
\item \textbf{The manifold is curved but smooth.} A nonlinear reduction
(autoencoder / kernel-POD) is the \emph{eventual} route to lower error,
but at 20 samples it would overfit; the target latent dimension is
${\sim}2$--3.
\end{enumerate}

\paragraph{Recommended next step.} \textbf{Add ${\sim}5$--8 samples in the
high-$|\opin|$/low-$\vadv$ corner} and along the convex-hull edges, where
the LOOCV error concentrates. Adding the previously-missing case (which
sits in that corner) already reduced the worst-case density error from
${\sim}48\%$ to ${\sim}43\%$ --- direct empirical support for this
recommendation.
...
\texttt{run\_rom\_first\_stage.py}, \texttt{run\_rom\_manifold.py}, and
