#!/usr/bin/env python3
Production Particle Tracking Workflow with HIERARCHICAL 5-HOP SEARCH

Implements full-scale particle tracking with:
- 105,000 particles (uniform grid: 50×70×30)
- 2,500 timesteps
- Hierarchical early-exit 5-hop search (memory-efficient)
...
- Uses hierarchical early-exit 5-hop search instead of 3-hop concatenated
- Memory: ~10 MB (vs 35 MB for 3-hop, vs 572 MB OOM for naive 5-hop)
- Hit rate: 99.99% (vs 99.9% for 3-hop)
- Expected retention: 82% (vs 16% for 3-hop)
- Expected throughput: 8-15k p/s (vs 23k for 3-hop)
...
OUTPUT_DIR = Path("./output/threadeda_hierarchical_5hop")  # Separate folder for optimized run
...
RK4_L1_HOP_COUNT = 5  # HIERARCHICAL: 5-hop early-exit for 82% particle retention
