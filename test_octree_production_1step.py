#!/usr/bin/env python3
"""
Production Particle Tracking Workflow - 3-Hop L1 + L2 Octree Fallback

Implements full-scale particle tracking with three-tier search hierarchy:
- 105,000 particles (uniform grid: 50x70x30)
- 2,500 timesteps
- Three-tier search: L0 + L1 (3-hop) + L2 (octree fallback)
- Async VTK export (no impact on tracking performance)
- Complete resource monitoring

Three-Tier Search Architecture:
- L0: Check cached elements (vmap, 85-95% hit rate)
- L1: Hierarchical 3-hop neighbor search (vmap + early exit, 99.9% cumulative)
  - Hop 1: 4 neighbors
  - Hop 2: 16 neighbors (if not found at hop 1)
  - Hop 3: 64 neighbors (if not found at hop 2)
- L2: Scan-based octree fallback (99.99% cumulative hit rate)
  - Level-filtered octree (refined regions only)
  - Fixed-depth scan with early exit
  - GPU-native JAX implementation

Expected Performance:
- Hit rate: 99.99% (vs 99.9% for 3-hop only)
- Retention: 82% at 2,500 steps (vs 16% for 3-hop only)
- Throughput: 40-48k p/s (minimal overhead <1%)
- Memory: +2 MB for octree (vs 3-hop baseline)

Advantages over 4-hop/5-hop hierarchical:
- Faster L1 search (3-hop vs 4-hop/5-hop)
- L2 octree catches rare misses efficiently
- Same retention as 5-hop with better throughput

Background Thread:
- Async VTK file writing
...
from jaxtrace.gpu.search.octree_builder import build_octree_for_level, flatten_octree_to_arrays, print_octree_stats
...
PARTICLE_GRID_RESOLUTION = (30, 50, 30)  # 10,000 particles  # Grid resolution in (x, y, z)
PARTICLE_BOUNDS_FRACTION = {
    'x': (0.3, 0.6),  # Use first 30% of domain in X
    'y': (0.0, 1.0),  # Full domain in Y
    'z': (0.0, 1.0),  # Full domain in Z
}

# Time Integration
N_TIMESTEPS = 1  # Single timestep
DT = 0.0025  # seconds
...
OUTPUT_DIR = Path("./output/threadeda_3hop_l2_octree")  # 3-hop + L2 octree test output
...
# Number of hops for hierarchical early-exit neighbor search (pure GPU, no CPU fallback)
# - 2: ~20 neighbors (95-98% hit rate, ~40k p/s, fastest, 16% retention)
# - 3: ~84 neighbors (98-99.5% hit rate, ~15-20k p/s, baseline, 16% retention WITHOUT L2)
# - 4: ~15 neighbors avg (hierarchical early-exit, 99.95% hit rate, ~12-18k p/s, 60% retention)
# - 5: ~25 neighbors avg (hierarchical early-exit, 99.99% hit rate, ~8-15k p/s, 82% retention)
# With L2 octree fallback: 3-hop L1 + L2 → 99.99% hit rate, 82% retention, 40-48k p/s
RK4_L1_HOP_COUNT = 3  # 3-hop L1 + L2 octree for optimal speed and retention

# L2 Octree Fallback Search (only used if USE_GPU_FUSED_RK4=True)
# False = L0 + L1 only (PHASE3A baseline, 99.9% hit rate, 16% retention)
# True  = L0 + L1 + L2 octree (99.99% hit rate, 82% retention, minimal overhead <1%)
#
# L2 Octree Architecture:
#   - Level-filtered octree (refined regions only, ~300k elements vs 3.5M total)
#   - Fixed-depth scan with early exit (GPU-friendly)
#   - Scan-based traversal (no data-dependent branches)
#   - Memory: +2 MB for octree metadata
#   - Performance: <1% overhead (only 0.05% of particles need L2)
USE_L2_OCTREE_FALLBACK = True  # Enable L2 octree for 82% retention
...
OCTREE_MAX_LEAF_SIZE = 50  # Maximum elements per leaf node (reduced from 500 to avoid GPU OOM)

# Block-local fallback (DISABLED - using L2 octree instead)
# This was an earlier approach, now superseded by L2 octree
...
# Build L2 Octree (if enabled)
octree_metadata_gpu = None
octree_elements_gpu = None

if USE_GPU_FUSED_RK4 and USE_L2_OCTREE_FALLBACK:
    print()
    print("=" * 80)
    print("L2 OCTREE CONSTRUCTION")
    print("=" * 80)
    print()
    ...
    if level_field is not None:
        ...
        print("⚠️  WARNING: No LEVEL field found in mesh")
        print("  L2 octree will be DISABLED (falling back to L0+L1 only)")
        print("  Expected retention: 16% (vs 82% with L2 octree)")
...
if USE_GPU_FUSED_RK4:
    ...
    if USE_L2_OCTREE_FALLBACK and octree_metadata_gpu is not None:
        print(f"  With L2 octree fallback (level-filtered scan-based search)")
        print(f"  Expected: 99.99% hit rate, 82% retention at 2,500 steps")
    else:
        print(f"  No L2 fallback (L0+L1 only)")
        print(f"  Expected: 99.9% hit rate, 16% retention at 2,500 steps")
...
print("  Note: Using small batch (1,000 particles) for JIT warm-up to avoid GPU OOM")
t0 = time.perf_counter()

# Initialize RK4 step function variable (will be set in warm-up if using L2 octree)
rk4_step_func = None

if USE_GPU_FUSED_RK4:
    # Warm up GPU-fused RK4 with SMALL BATCH to avoid OOM during JIT compilation
    # For hierarchical 5-hop, we need to reduce batch size during compilation
    warmup_batch_size = 80000  # Small batch for JIT compilation
    ...
    if USE_L2_OCTREE_FALLBACK and octree_metadata_gpu is not None:
        # Create L2 octree RK4 function (factory pattern - created ONCE)
        ...
        # Fallback to hierarchical only (no L2 octree)
        from jaxtrace.gpu.tracking.rk4_gpu_fused import rk4_step_gpu_fused_for_production_hierarchical

        _, _ = rk4_step_gpu_fused_for_production_hierarchical(
            warmup_data,  # Use small batch for JIT warm-up
...
for step in range(N_TIMESTEPS):
    ...
    if USE_GPU_FUSED_RK4:
        ...
        if USE_L2_OCTREE_FALLBACK and octree_metadata_gpu is not None:
            # Three-tier search: L0 (cached) + L1 (3-hop) + L2 (octree)
        ...
        elif USE_BLOCK_LOCAL_FALLBACK:
            # Two-tier search with block fallback: L0 (cached) + L1 (n-hop) + Block fallback
            ...
            # Two-tier search only: L0 (cached) + L1 (n-hop hierarchical)
            from jaxtrace.gpu.tracking.rk4_gpu_fused import rk4_step_gpu_fused_for_production_hierarchical

            particle_data, rk4_stats = rk4_step_gpu_fused_for_production_hierarchical(
                particle_data,
