#!/usr/bin/env python3
"""
Test mesh-aligned Morton with SMART multi-cell assignment.

SMART approach:
- Assigns element only to cells containing its vertices
- Expected: ~1-4 cells/element (vs 27 for naive bbox)
- Same coverage, much less overhead
"""
...
from jaxtrace.gpu.search.mesh_aligned_octree_smart_multi import extract_octree_cells_smart_multi
...
# Search method
SEARCH_METHOD = 'grid'  # 'grid' or 'morton'
SEARCH_RADIUS = 1  # Grid: 1 = 3×3×3 = 27 cells, Morton: 1 = 3 cells

config.POINT_IN_TET_METHOD = 'inverse'

print("=" * 80)
print(f"Mesh-Aligned Morton - SMART Multi-Cell Assignment")
print(f"Search method: {SEARCH_METHOD.upper()}, radius={SEARCH_RADIUS}")
print("=" * 80)
...
# Extract cells with SMART multi-cell assignment
print("\n[3/7] Extracting octree cells (SMART multi-cell)...")
mesh_octree_cells = extract_octree_cells_smart_multi(
    node_positions, connectivity, tolerance=1e-6, verbose=True
)

# Build Morton structure
print("\n[4/7] Building mesh-aligned Morton structure...")
...
# Upload to GPU
print("\n[7/7] Uploading to GPU and testing search...")
...
# Search
print(f"  Searching with {SEARCH_METHOD} method, radius={SEARCH_RADIUS}...")
...
print(f"\n{'='*80}")
print(f"RESULTS - SMART MULTI-CELL")
print(f"{'='*80}")
...
print(f"  Actual success rate (includes void): {success_rate:.1f}%")

if SEARCH_METHOD == 'grid':
    print(f"\n  Note: Grid search finds TRUE spatial neighbors (3D cube)")
else:
    print(f"\n  Note: Morton search follows 1D curve")
...
print(f"\n{'='*80}")
print(f"✅ SMART multi-cell test complete!")
print(f"{'='*80}\n")
