...
NEIGHBOR_METHOD = 'face'       # 'face' or 'node' - Choose based on mesh structure
...
#             - Performance: ~13K particles/s with radius=10, 79% retention
...
# NOTE: 'hierarchical' causes severe performance degradation (1.4K p/s vs 21K p/s)
#       due to nested loops (27 octants × 8 leaves × 2 depths = 432 iterations)
#       Use 'neighbors' with improved multi-leaf search (up to 3 leaves) instead
...
if CURVE_TYPE == 'hilbert':
    INITIAL_SEARCH_RADIUS = 75         # ~1.5× Morton radius
    INITIAL_SEARCH_FALLBACK_RADII = [150, 300, 600]  # ~1.5× Morton fallbacks
else:  # morton
